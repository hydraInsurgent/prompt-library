...
When the plan has a `## UI Specification` or `## UI Decisions` section:

1. **Read the linked UI-SPEC.md file** at the start of execution
2. **The UI-SPEC is the single source of truth** for design decisions - do not read the reference files in `.claude/ui-reference/` directly
